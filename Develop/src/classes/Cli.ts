...
class Cli {
      ...
      .prompt([
        {
          ...
          choices: this.vehicles.map(
            (vehicle: { vin: any; make: any; model: any }) => {
              return {
  ...
  findVehicleToTow(): void {
    inquirer
      ...
      .then(
        (answers: { vehicleToTow: { vin: any; make: any; model: any } }) => {
          ...
          if (answers.vehicleToTow.vin === this.selectedVehicleVin) {
            ...
            console.log(
              `The ${answers.vehicleToTow.make} ${answers.vehicleToTow.model} is ready to tow`
            );
